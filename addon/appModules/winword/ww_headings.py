...
import gui
import ui
from NVDAObjects.window.winword import WordDocument, wdGoToNext,wdGoToHeading
import time
from eventHandler import queueEvent
from .ww_wdConst import wdActiveEndPageNumber , wdFirstCharacterLineNumber , wdGoToHeading
...
class Heading(CollectionElement):
	...
	""")
		sInfo = sInfo.replace("\n", "\r\n")
		return sInfo.format(page =self.page, line= self.line, text = self.text, level = self.level, style = self.style)
...
class Headings(Collection):
	...
	def __init__(self, parent,obj, rangeType):
		...
		super(Headings, self).__init__( parent,obj)
	...
	def getCollectionInRange(self, theRange):
		...
		while True:
			...
			if i > 400 or not newRangeObj or newRangeObj.start>theRange.End or newRangeObj.Start == rangeObj.Start:
				break
