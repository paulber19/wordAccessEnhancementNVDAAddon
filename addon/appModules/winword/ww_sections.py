...
{pageSetup}
""").format(pageStart =self.pageStart, lineStart= self.lineStart,pageEnd = self.pageEnd, lineEnd = self.lineEnd, footer = footer, header = header, protected = protected, pageSetup = pageSetup)
		sInfo = sInfo.replace("\n", "\r\n")
		return sInfo
	def reportText(self):
...
class Sections(Collection):
	...
	def __init__(self, parent, obj, rangeType):
		...
		super(Sections, self).__init__( parent, obj)
...
class PageSetup(object):
	def __init__(self, parent, item):	
		...
		self.oddAndEvenPagesHeaderFooter    = item.OddAndEvenPagesHeaderFooter    #true if has different headers and footers 
		#for odd-numbered and even-numbered pages. Can be True, False, or wdUndefined.  
		self.differentFirstPageHeaderFooter    = item.DifferentFirstPageHeaderFooter    
		self.footerDistance    = item.FooterDistance   # distance between footer and bottom of the page(in points)
		self.headerDistance = item.HeaderDistance # distance between header and top of the page (in points)
		...
		self.twoPagesOnOne    = item.TwoPagesOnOne    #True if Microsoft Word prints the specified document 
			#two pages per sheet.  
...
""") .format(start = start, oddAndEven = oddAndEven
			, firstPage = firstPage, headerDistance = headerDistance, footerDistance = footerDistance, verticalAlignment = verticalAlignment)
		#margins
...
""")).format(left= left, right= right, top = top, bottom = bottom, orientation = orientation)
		#paper
...
""")) .format(paperSize = paperSize, height = height, width= width, twoPages = twoPages)
		# page
...
{paper}
{pageFormat}
""")).format(disposition = disposition, margins = margins, paper = paper, pageFormat= pageFormat)
		sInfo = sInfo.replace("\n", "\r\n")
		return sInfo
class SectionsDialog(ReportDialog):
	...
	def get_lcColumnsDatas(self, element):
		start = (_("Page {page}, line {line}")).format(page = element.pageStart, line= element.lineStart)
		end = (_("Page {page}, line {line}")).format(page = element.pageEnd, line= element.lineEnd)
