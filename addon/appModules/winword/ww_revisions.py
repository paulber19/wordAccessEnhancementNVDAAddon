...
from .ww_wdConst import wdActiveEndPageNumber , wdFirstCharacterLineNumber , wdGoToRevision
...
class Revision(CollectionElement):
	...
	def accept(self):
		#doc = self.parent.doc
		#revision = doc.revisions[self.indexInDocumentCollection]
		#revision.accept()
		self.obj.accept()
...
""")
		...
		sInfo = sInfo.replace("\n", "\r\n")
		return sInfo.format(page =self.page, line= self.line, text = self.text, author = self.author, date = self.date, type = self.get_typeText())

	def reportText(self):
		ui.message(_("{type} by {author} {text}") .format(type = self.get_typeText(), author = self.author, text = self.text)		)
...
class Revisions(Collection):
	...
	def __init__(self, parent, obj, rangeType):
		...
		super(Revisions, self).__init__( parent, obj)
...
class RevisionsDialog(ReportDialog):
	...
	def onInputChar(self, evt):
		...
		if (key == wx.WXK_RETURN):
			...
			if gui.messageBox(_("Are you sure you wish to reject this items?"), _("Reject of items"), wx.YES_NO|wx.ICON_WARNING) != wx.YES: return
			self.reject()
			return
