...
from .ww_wdConst import wdActiveEndPageNumber , wdFirstCharacterLineNumber , wdStoryValueToText, wdGoToBookmark
...
class Bookmark(CollectionElement):
	def __init__(self, parent, item):
		...
		if item.Name:
			self.name = item.Name
		self.column = item.column # a value that indicates if the specified bookmark is a table column. 
		self.empty = item.empty # a value that indicates if the specified bookmark is empty. 
...
class Bookmarks(Collection):
	...
	def __init__(self, parent, obj, rangeType):
		...
		super(Bookmarks, self).__init__(parent, obj)
