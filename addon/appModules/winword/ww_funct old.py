...
def sayActivePane (  ):
	oFocus = api.getFocusObject()
	hFocus = oFocus.windowHandle
	#If ((( isstatusbartoolbar ( hFocus) || intaskpanedialog () ) || inribbons () ) || isvirtualribbonactive () ) || dialogactive ():
		#Return
	...
	if storyType:
		...
		try:
			...
			oFrameset = None
		if oFrameset:
			ui.message(  _("Frame {name} document pane {index}") .format(name = oFrameset.FrameName , index = str( oActivePane.index)))
		elif  sPaneName != "":
