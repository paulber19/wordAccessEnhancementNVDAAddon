...
class FormatFieldSpeech(textInfos.TextInfo):
	def getFormatFieldSpeech(
		...
		*args, **kwargs
		...
		if NVDAVersion >= [2026, 1]:
			funct = self.getFormatFieldSpeech_2026_1
		else:
			funct = self.getFormatFieldSpeech_2025_1
		seq = funct(
			*args, **kwargs
		)
		return seq

	def getFormatFieldSpeech_2025_1(  # noqa: C901
		self,
		...
		if formatConfig["reportRevisions"]:
			...
			if (revision or oldRevision is not None) and revision != oldRevision:
				...
				if (
					...
					and _addonConfigManager.toggleAutoInsertedTextReadingOption(False)
				):
					from ..automaticReading import formatRevisionAutoReadingSequence
			...
			if (revision or oldRevision is not None) and revision != oldRevision:
				...
				if _addonConfigManager.toggleAutomaticReadingOption(False) and (
					...
					from ..automaticReading import formatDeletedRevisionAutoReadingSequence
	...
	def getFormatFieldSpeech_2026_1(  # noqa: C901
		self,
		attrs: textInfos.Field,
		attrsCache: Optional[textInfos.Field] = None,
		formatConfig: Optional[Dict[str, bool]] = None,
			...
			if (invalidSpelling or oldInvalidSpelling is not None) and invalidSpelling != oldInvalidSpelling:
				...
				if invalidSpelling:
					...
					if formatConfig["reportSpellingErrors2"] & ReportSpellingErrors.SPEECH.value:
						# Translators: Reported when text contains a spelling error.
						texts.append(NVDAString("spelling error"))
				elif extraDetail:
					# Translators: Reported when moving out of text containing a spelling error.
					texts.append(NVDAString("out of spelling error"))
